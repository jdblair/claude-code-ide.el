...
- Session buffers are still named =*claude-code[dir]*= regardless of the
  active agent; MCP session ids use a =pi-<dir>-<ts>= prefix for pi.
- =claude-code-ide-cli-debug= maps to pi's =--verbose= flag, which only
  increases startup verbosity.
- Buffer names, side-window layout, and all window/session commands behave
